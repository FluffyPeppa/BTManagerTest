...
public class BleScanner {
	...
	public void startScanning(final ScanResultsConsumer scan_results_consumer, long stop_after_ms) {
		...
		// advertising with a device name of “iPhone” will be passed to the MainActivity class for listing on the UI.
		// Comment out next 2 lines to see all ble peripherals

		//ScanFilter filter1 = new ScanFilter.Builder().setManufacturerData(0x004c, new byte[] {}).build();

		ScanFilter filter2 = new ScanFilter.Builder().setDeviceName("Michelle").build();

		//ScanFilter filter3 = new ScanFilter.Builder().setServiceUuid(  )
		//ScanFilter filter4 = new ScanFilter.Builder().setDeviceAddress(  )
		//filters.add(filter1);
		filters.add(filter2);
		//filters.add(filter3);
		//filters.add(filter4);
		...
		//bluetooth_adapter.getBluetoothLeScanner().startScan(filters, settings, scan_callback);
	}
